...
if (bannerView != null && !isInternetAvailable)
{
    DestroyBannerAd();
}

if (isInternetAvailable && (bannerView == null))
{
    RequestBannerAd();
}
   ...
   public void RequestBannerAd()
{
    ...
    if (bannerView == null && PlayerPrefs.GetInt("removeads") == 0)
    {

        if (!isInternetAvailable)
    {
        Debug.Log("No internet available. Cannot request banner ad.");
        return;
    }
        PrintStatus("Requesting Banner ad.");

        // Use the appropriate test ad unit ID for the platform
        string adUnitId = "ca-app-pub-3940256099942544/6300978111";  // Android test ad unit ID
    ...
    public void ShowInterstitialAd()
    {
        ...
        if (isInternetAvailable)
        {
         ...
         if (interstitialAd.IsLoaded() && PlayerPrefs.GetInt("removeads") == 0)
         {
        Debug.Log("war gya");
    ...
    public void ShowRewardedAd(Action<Reward> rewardCallback)
    {
     ...
     if (isInternetAvailable)
     {
        ...
        {
            // Hook up the event handler for the ad completion
            rewardedAd.OnUserEarnedReward += (sender, args) => HandleUserEarnedReward(sender, args, rewardCallback);
        ...
        else
        {
            ...
            Debug.Log("Rewarded ad is not loaded. Loading now...");
            RequestAndLoadRewardedAd();
        }
        Invoke("Delayreward", 0.5f);
     }
     else{
        ToastManager.instance.ShowToast("Ad is Not Available");
     }
    ...
    private void HandleUserEarnedReward(object sender, Reward args, Action<Reward> rewardCallback)
    {
        // This method will be called when the user has completed watching the full video.
        Debug.Log("User earned reward. Type: " + args.Type + ", amount: " + args.Amount);

        // Invoke the callback with the earned reward
        rewardCallback?.Invoke(args);

        // Clean up event handler to avoid multiple calls
    ...
    public void ShowAppOpenAd()
    {
        ...
        if (appOpenAd != null  && PlayerPrefs.GetInt("removeads") == 0)
        {
